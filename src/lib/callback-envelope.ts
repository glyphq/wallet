...
import { CALLBACK_ENVELOPE_VERSION, bytesToBase64, jcsCanonicalize, sha256Base64Url } from "@/lib/jcs";
...
function envelopeField<T>(envelope: GlyphEnvelope, key: string, fallback: T): T {
  const value = (envelope as unknown as Record<string, unknown>)[key];
  return (value as T | undefined) ?? fallback;
}

async function requestHash(envelope: GlyphEnvelope): Promise<string> {
  const existing = envelopeField<string | null>(envelope, "request_hash", null);
  ...
  return `sha256:${await sha256Base64Url(jcsCanonicalize(requestBinding))}`;
}

function safeEpochSeconds(epochSeconds: number): number {
  if (!Number.isSafeInteger(epochSeconds) || epochSeconds < 0) {
    throw new Error("callback timestamp is invalid");
  }
  return epochSeconds;
}
...
export async function buildCallbackSignaturePayload(
  ...
  networkId = "qubic:mainnet",
): Promise<CallbackSignaturePayload> {
  return {
    version: CALLBACK_ENVELOPE_VERSION,
    request_hash: await requestHash(envelope),
    network: envelopeField(envelope, "network", { id: networkId }),
...
export async function buildSignedCallbackEnvelope(input: {
  ...
  ) => Promise<{ signature: Uint8Array; publicKey: Uint8Array; identity: string }>;
  networkId?: string;
  ...
  const signedPayload = jcsCanonicalize(payload);
